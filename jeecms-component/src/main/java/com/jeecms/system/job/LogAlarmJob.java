 ...
 */
public class LogAlarmJob implements IBaseJob {

	private static final Logger logger = LoggerFactory.getLogger(LogAlarmJob.class);
			...
			if (emails != null) {
				try {
					messageService.sendMemberMsg(null, null, null, null, null, null, MessageSceneEnum.USER_MESSAGE,
							MessageTempleEnum.LOG_DANGER_TEMP.getTitle(), content(dangerValue), null,
							Arrays.asList(emails), new JSONObject(), MqConstants.SEND_EMAIL, null);
				} catch (GlobalException e) {
					logger.error("告警邮件发送错误，错误{}", e.getMessage());
				}
			}
		}
	}
			...
			if (phones != null) {
				try {
					messageService.sendMemberMsg(null, null, null, null, null, dangerSmsTmpId,
							MessageSceneEnum.USER_MESSAGE, MessageTempleEnum.LOG_DANGER_TEMP.getTitle(),
							content(dangerValue), Arrays.asList(phones), null, new JSONObject(), MqConstants.SEND_SMS,
							null);
				} catch (GlobalException e) {
					logger.error("告警短信发送错误，错误{}", e.getMessage());
				}
			}
		}
	}
	...
	private String content(Integer dangerValue) {
		...
		return String.format(MessageTempleEnum.LOG_DANGER_TEMP.getContent(), url, database, dangerValue);
	}

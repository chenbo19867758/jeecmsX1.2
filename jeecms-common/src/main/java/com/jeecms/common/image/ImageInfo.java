 ...
 * PNG, IFF, RAS, PBM, PGM, PPM and PSD files (or input streams). Use the class
 * like this:
 * 
 * <pre>
 ...
 * image-info/</a>. Written by Marco Schmidt. This class is contributed to the
 * Public Domain. Use it at your own risk. <a name="history">History</a>:
 * <ul>
...
public class ImageInfo {
	...
	private boolean checkBmp() throws IOException {
		...
		if (bitsPerPixel != 1 && bitsPerPixel != 4 
				&& bitsPerPixel != 8 && bitsPerPixel != 16 && bitsPerPixel != 24
				&& bitsPerPixel != 32) {
			return false;
		}
	...
	private boolean checkIff() throws IOException {
		...
		final byte[] iefrm = { 0x52, 0x4d };
		if (!equals(a, 0, iefrm, 0, 2)) {
			return false;
		}
	...
	private boolean checkPcx() throws IOException {
		...
		int bits = a[1];
		int planes = a[63];
		boolean bitPerPixelBit = planes == 1 && (bits == 1 || bits == 2 || bits == 4 || bits == 8);
		if (bitPerPixelBit) {
			// paletted
			bitsPerPixel = bits;
	...
	private boolean checkPnm(int id) throws IOException {
		...
		final int[] pnmFormat = { FORMAT_PBM, FORMAT_PGM, FORMAT_PPM };
		format = pnmFormat[(id - 1) % 3];
			...
			while (index < args.length) {
				...
				try {
					...
					if (name.startsWith("http://")) {
						...
						name = java.text.Normalizer.normalize(name, java.text.Normalizer.Form.NFKD);

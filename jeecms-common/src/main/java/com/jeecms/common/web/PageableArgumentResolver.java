...
public class PageableArgumentResolver implements HandlerMethodArgumentResolver {
	...
	private static final Pageable DEFAULT_PAGE_REQUEST = PageRequest.of(0, DEFAULT_PAGE_SIZE);
	...
	/**
	 * Configures the maximum page size to be accepted. This allows to put an
	 * upper boundary of the page size to prevent potential attacks trying to
	 * issue an {@link OutOfMemoryError}. Defaults to
	...
	/**
	 * Retrieves the maximum page size to be accepted. This allows to put an
	 * upper boundary of the page size to prevent potential attacks trying to
	 * issue an {@link OutOfMemoryError}. Defaults to
	 * {@link #DEFAULT_MAX_PAGE_SIZE}.
	 * 
	...
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
		...
		String pageSizeStr = servletRequest.getParameter(DEFAULT_SIZE);
		if (StringUtils.isNotBlank(pageSizeStr)) {
			pageSize = NumberUtils.toInt(pageSizeStr, 0);
		}
		if (pageSize <= 0) {
		...
		request = PageRequest.of(pageNumber, pageSize, request.getSort());
		return request;
	}
	...
	private Pageable getDefaultFromCookieOrAnnotationOrFallback(MethodParameter methodParameter,
		...
		return PageRequest.of(fallbackPagable.getPageNumber(), pageSize, fallbackPagable.getSort());
	}
	...
	private static Pageable getDefaultPageRequestFrom(PageableDefault defaults, Integer pageSize) {
		...
		return PageRequest.of(defaultPageNumber, pageSize, defaults.direction(), defaults.sort());
	}
	...
	private void assertPageableUniqueness(MethodParameter parameter) {
		...
		if (method != null && containsMoreThanOnePageableParameter(method)) {
			...
			if (Pageable.class.equals(parameterTypes[i])) {
				...
				if (null == qualifier) {
					throw new IllegalStateException(
							"Ambiguous Pageable arguments in handler method. "
							+ "If you use multiple parameters of type Pageable"

...
public class ResourceType {
	 ...
	 * 附件
	 */
	public static final Short RESOURCE_TYPE_ANNEX = 4;
	/**
	 * 音频
	 */
	...
	private static Map<Short, ResourceType> shortTypes = new HashMap<Short, ResourceType>(4);

	/** Resource type <code>File</code> */
	public static final ResourceType FILE = new ResourceType("File", PropertiesLoader.getFileResourceTypePath(),
			Utils.getSet(PropertiesLoader.getFileResourceTypeAllowedExtensions()),
			Utils.getSet(PropertiesLoader.getFileResourceTypeDeniedExtensions()));
	/** Resource type <code>Flash</code> */
	public static final ResourceType FLASH = new ResourceType("Flash", PropertiesLoader.getFlashResourceTypePath(),
			Utils.getSet(PropertiesLoader.getFlashResourceTypeAllowedExtensions()),
			Utils.getSet(PropertiesLoader.getFlashResourceTypeDeniedExtensions()));
	/** Resource type <code>Image</code> */
	public static final ResourceType IMAGE = new ResourceType("Image", PropertiesLoader.getImageResourceTypePath(),
			Utils.getSet(PropertiesLoader.getImageResourceTypeAllowedExtensions()),
			Utils.getSet(PropertiesLoader.getImageResourceTypeDeniedExtensions()));
	/** Resource type <code>Media</code> */
	public static final ResourceType MEDIA = new ResourceType("Media", PropertiesLoader.getMediaResourceTypePath(),
			Utils.getSet(PropertiesLoader.getMediaResourceTypeAllowedExtensions()),
			Utils.getSet(PropertiesLoader.getMediaResourceTypeDeniedExtensions()));
	
	public static final ResourceType AUDIO = new ResourceType("Audio", PropertiesLoader.getAudioResourceTypePath(),
			Utils.getSet(PropertiesLoader.getAudioResourceTypeAllowedExtensions()),
			Utils.getSet(PropertiesLoader.getAudioResourceTypeDeniedExtensions()));
	...
	static {
		...
		shortTypes.put(RESOURCE_TYPE_ANNEX, FILE);
		shortTypes.put(RESOURCE_TYPE_VIDEO, MEDIA);
		shortTypes.put(RESOURCE_TYPE_IMAGE, IMAGE);
		shortTypes.put(RESOURCE_TYPE_AUDIO, AUDIO);
	}
	 ...
	 */
	private ResourceType(final String name, final String path, final Set<String> allowedEextensions,
		...
		if (!allowedEextensions.isEmpty() && !deniedExtensions.isEmpty()) {
			throw new IllegalArgumentException("Both sets contain extensions, "
					+ "only one can be filled at the same time");
		}
	 ...
	 * the userfiles path. To set this path, see the
	 * <a href="http://java.fckeditor.net/properties.html">configuration</a>.
	 * 
	...
	public static ResourceType valueOf(final Short resType) {
		if (resType==null) {
			throw new NullPointerException("Name is null or empty");
		}
		ResourceType rt = shortTypes.get(resType);
		if (rt == null) {
			throw new IllegalArgumentException("No resource type const " + resType);
		}
		return rt;
	}
